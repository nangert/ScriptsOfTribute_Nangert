...
class BetterNetV16(nn.Module):
    def __init__(
        ...
        self.policy_proj = nn.Linear(hidden_dim * 2, hidden_dim * 2)
    ...
    def forward(
        ...
        shape = len(obs["current_player"].shape)
        if shape == 3:
            ...
            hidden_proj = self.policy_proj(lstm_out)
            logits = self.policy_head(hidden_proj)

            return logits, values

        elif shape == 2:
            B, T = obs["tavern_available_ids"].shape
            ...
            hidden_proj = self.policy_proj(final_hidden)
            logits = self.policy_head(hidden_proj)
            ...
            raise ValueError(
                f"Unexpected move_tensor.dim()={shape}; expected 3 or 4."
            )
